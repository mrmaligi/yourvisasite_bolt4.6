...
export function UnifiedLogin() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Redirect happens automatically by Supabase/AuthContext
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fetch profile (AuthContext handles this too, but we need it for routing logic immediately)
      ...
      if (!profile) {
        // Profile not created yet? Wait a moment or redirect
        // AuthContext will likely trigger creation or fetch soon
        console.warn('Profile not found immediately, redirecting to dashboard');
        navigate('/dashboard');
        return;
      }
      ...
      switch (profile.role) {
        ...
        case 'lawyer': {
          ...
          // Use 'any' cast if TS complains about dynamic join result structure,
          // though supabase-js types usually handle this if generated.
          // Checking status safely.
          const status = (lawyerProfile as any)?.verification_status;
