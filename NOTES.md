...
*   **Usage:** This data is used by the `AuthContext` to simulate a logged-in user and by various pages to populate lists.

## 2. Authentication Bypass (Mock Auth)
*   **File Modified:** `src/contexts/AuthContext.tsx`
*   **Change:** Added a `USE_MOCK` constant (set to `true`).
...
1.  **AuthContext:** In `src/contexts/AuthContext.tsx`, set `const USE_MOCK = false;` (or remove the mock logic).
2.  **Components:** In the modified pages (`UserManagement.tsx`, `LawyerDirectory.tsx`, `LawyerManagement.tsx`), set `const USE_MOCK = false;` (or remove the mock logic block).
3.  **Header:** Revert changes to `src/components/Header.tsx` to restore the original Login/Register links and remove the Dev Switcher. (You can use `git checkout src/components/Header.tsx` if using git tracking).
