...
## 2025-03-06 - Input, Textarea, and Select accessibility (aria-invalid & aria-describedby)
**Learning:** Found that custom form components (`Input`, `Textarea`, `Select`) in `src/components/ui/Input.tsx` lacked `aria-invalid` to announce validation state to screen readers, and lacked `aria-describedby` to programmatically associate error or helper texts with the input field. Also, the fallback ID generation (`label.toLowerCase().replace(...)`) was not robust enough and could produce non-unique IDs.
**Action:** Replaced manual ID generation with React's robust `useId` hook. Added `aria-invalid={!!error}` to all inputs. Used `useId` to generate unique IDs for the error and helper text elements and linked them using `aria-describedby`. Prioritized `errorId` over `helperId` when associating with `aria-describedby` to ensure error messages take precedence.
